...
u8 time_err;
u8 test_buff[40970], CAN_need_update_data[8], temp_data[8];
u8 needupdate, CAN_need_update = 12, temp_id;
u32 down_edge_time, up_edge_time, down_edge_count, up_edge_count;
extern u8 Vbcan_pending;
void hit_event(u8 i);
extern u32 DAQ_id[4];
...
void USB_LP_CAN1_RX0_IRQHandler(void)
{
   ...
   RxMessage.RTR=CAN_RTR_DATA;  // 数据帧or远程
    ...
    for(i =0; i < 8; i++)
    {
      temp_data[i] = RxMessage.Data[i];
    }
    if(Vbcan_pending == 0)
    {
       Vbcan_pending = 1;
       CAN_need_update = temp_id;
       for(i =0; i < 8; i++)
       {
          CAN_need_update_data[i] = temp_data[i];
       }
    }
